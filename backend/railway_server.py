"""
Railway-optimized server configuration for GRRAS Solutions Training Institute
This file contains the main FastAPI server configured for Railway deployment
"""
...
# MongoDB connection with Railway environment support
...
# Create directories with Railway-compatible paths
BACKEND_DIR = Path(__file__).parent
os.makedirs(BACKEND_DIR / 'storage', exist_ok=True)
os.makedirs(BACKEND_DIR / 'temp', exist_ok=True)
os.makedirs(BACKEND_DIR / 'data' / 'media', exist_ok=True)
os.makedirs(BACKEND_DIR / 'data' / 'versions', exist_ok=True) 
os.makedirs(BACKEND_DIR / 'data' / 'backups', exist_ok=True)
...
class StorageService:
    def __init__(self):
        ...
        self.json_file = BACKEND_DIR / 'storage' / 'leads.json'
...
# Models (keeping all the existing models)
...
# PDF Generation with Railway-compatible paths
async def generate_syllabus_pdf(course_slug: str, student_name: str) -> str:
    ...
    try:
        ...
        # Create temporary file with Railway-compatible path
        temp_file = BACKEND_DIR / 'temp' / f"syllabus_{course_slug}_{uuid.uuid4().hex[:8]}.pdf"
        
        # Create PDF
        doc = SimpleDocTemplate(str(temp_file), pagesize=A4, topMargin=1*inch)
...
async def admin_login(login_data: AdminLogin, response: Response):
    ...
    # Set httpOnly cookie (secure=True for Railway HTTPS)
    response.set_cookie(
        key="admin_token",
        value=token,
        httponly=True,
...
# Mount static files for media with Railway-compatible path
app.mount("/media", StaticFiles(directory=str(BACKEND_DIR / "data" / "media")), name="media")

# CORS with Railway-compatible origins
...
app.add_middleware(
    ...
    allow_origins=railway_cors_origins,
    allow_methods=["*"],
    allow_headers=["*"],
)
