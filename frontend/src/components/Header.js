...
// 🔁 Static fallbacks (mobile/desktop logos consistent rakhne ke liye)
...
const Header = () => {
  ...
  // Use ContentContext if present; otherwise fall back to static files
  const sourceCategories = (content?.courseCategories && Object.keys(content.courseCategories).length)
    ? content.courseCategories
    : staticCategories;

  const sourceCourses = (Array.isArray(content?.courses) && content.courses.length)
    ? content.courses
    : staticCourses;
  ...
  // Build Technology Tracks for dropdown / mobile drawer
  const technologyTracks = Object.entries(sourceCategories)
    .filter(([, cat]) => cat?.visible !== false)
    .sort(([, a], [, b]) => (a.order || 999) - (b.order || 999))
    .map(([slug, category]) => {
      const courseCount = sourceCourses.filter(
      ...
      return {
        ...
        courseCount,
        // ✅ One source of truth for icons/logos (mobile + desktop)
        logo:
          category.logo_url ||
          category.logo ||
          staticCategories?.[slug]?.logo_url ||
          staticCategories?.[slug]?.logo ||
          null,
      };
    });
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <ChevronDown
                      className={`h-5 w-5 transition-transform duration-300 ${
                        mobileExpandedCategories.has('courses') ? 'rotate-180' : ''
                      }`}
                    />
                  </div>
                </button>
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {technologyTracks.length > 0 ? (
                ...
                <div className="text-center text-gray-500 py-6">
                  ...
                  <div className="text-xs mt-2">Total courses: {sourceCourses.length}</div>
                </div>
              )}
            </div>
