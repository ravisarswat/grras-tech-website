...
const Header = () => {
  ...
  return (
    <>
      {!isAdminPage && (
        <header className="bg-white/98 backdrop-blur-xl shadow-2xl border-b border-orange-100/50 sticky top-0 z-[9998] relative overflow-hidden">
                ...
                {filteredNavigationItems.map((item) => (
                  <div key={item.name} className="relative">
                    ...
                    {item.hasDropdown && isCoursesOpen && (
                      <div 
                        className="fixed top-20 left-0 right-0 w-full bg-white/99 backdrop-blur-2xl shadow-2xl border-t-4 border-orange-500 z-[99999] animate-in slide-in-from-top-4 duration-500 overflow-hidden"
                        onMouseEnter={() => setIsCoursesOpen(true)}
