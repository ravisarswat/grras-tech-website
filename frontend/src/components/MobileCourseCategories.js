...
// ✅ Use static data directly so mobile logos = desktop logos
import { categories as staticCategories } from '../data/categories';
import { courses as staticCourses } from '../data/courses';

const MobileCourseCategories = () => {
  // Default expand Red Hat (aap chahein to [] kar sakte ho)
  const [expanded, setExpanded] = useState(new Set(['red-hat-technologies']));

  // Featured categories with counts + top 3 courses
  const featuredCategories = Object.entries(staticCategories)
    .filter(([_, cat]) => cat.featured)
    .map(([slug, cat]) => {
      const categoryCourses = staticCourses.filter(
      ...
      return {
        ...
        topCourses: categoryCourses.slice(0, 3),
      };
    })
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="space-y-3">
          {featuredCategories.map((cat) => {
            ...
            const logoSrc = cat.logo_url || cat.logo || null;

            return (
              <div key={cat.slug} className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden transition-all duration-300">
                {/* Row */}
                <button
                  ...
                  <div className="border-t border-gray-100 bg-gray-50/50">
                    <div className="p-4 space-y-3">
                            ...
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex-1">
                                  ...
                                  <div className="flex items-center space-x-3 mt-1">
                                    <span className="text-xs text-gray-500">{course.duration || '4-6 Weeks'}</span>
                                    <span className="text-xs text-gray-500">{course.level || 'Professional'}</span>
                                    {course.fees && (
                                      <span className="text-xs font-medium text-orange-600">{course.fees}</span>
                                    )}
                                  </div>
                                </div>
                      ...
                      <Link
                        ...
                        className="block w-full mt-2 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white text-center rounded-lg font-semibold text-sm hover:from-orange-600 hover:to-red-600 transition-all duration-300 shadow-md hover:shadow-lg"
                      >
